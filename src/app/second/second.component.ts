...
//On opening or initialization of webpage, asks for user location to display weather conditions
  ngOnInit(){
    this.getCityWeather();
    this.cityName = this.route.snapshot.paramMap.get('cty');
...
//Obtain weather details in JSON format
  getCityWeather(){
    ...
    this.httpClient.get(api_url).subscribe(data=> {
      ...
      this.Maxtemp = (parseFloat(this.weatherJson.main.temp_max)-273.15).toFixed(1);
      this.Mintemp = (parseFloat(this.weatherJson.main.temp_min)-273.15).toFixed(1);
